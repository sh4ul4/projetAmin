...
__device__
inline int access(int a, int b, int c, int W, int H) {
  return (c*H + b)*W + a;
}

__device__
inline int minAccessiblePixels (int x, int y, int z,
...
__global__ 
inline void double_pass_square_CB(unsigned int* workImage,      //0
...
inline void callDoublePassSquare_CB(dim3 numBlocks, int modifWidth, int modifHeight, int modifDepth,
    unsigned int* workImage,
    unsigned int* nbModifiedBlock,
    unsigned int* connectImage,
    int  W, int  H, int  D,
    int  BLOCKSIZEX, int  BLOCKSIZEY, int  BLOCKSIZEZ
) {
    int N = modifWidth * modifHeight * modifDepth;
    dim3 threadsPerBlock = dim3(
        modifWidth / numBlocks.x,
        modifHeight / numBlocks.y,
        modifDepth / numBlocks.z);
    double_pass_square_CB << <numBlocks, threadsPerBlock >> > (workImage, nbModifiedBlock, connectImage, W, H, D, BLOCKSIZEX, BLOCKSIZEY, BLOCKSIZEZ);
}